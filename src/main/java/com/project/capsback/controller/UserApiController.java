...
public class UserApiController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<UserResponse> showUser(@PathVariable String id) { //특정 사용자의 정보 조회
        return ResponseEntity.ok(userService.findUserResponseById(id));
    }

    @PostMapping
    public ResponseEntity<UserResponse> createUser(@RequestBody UserRequest userRequest) { //사용자 신규 생성
